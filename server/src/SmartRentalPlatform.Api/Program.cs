...
if (args.Length > 0 && args[0] == "verify-showcase-seed")
{
    ...
    var summary = new[]
    {
        ...
        ("showcase_deposit_paid_transfer", "SELECT COUNT(*) FROM room_deposits d JOIN contracts c ON c.room_deposit_id = d.id WHERE c.contract_number = 'HD-XH-B201-20260601' AND d.status = 'Paid' AND d.payment_transfer_group_id IS NOT NULL AND d.refund_transfer_group_id IS NULL"),
        ...
        ("xunhuns_active_deposits_paid_transfer", "SELECT COUNT(*) FROM room_deposits d JOIN contracts c ON c.room_deposit_id = d.id JOIN rooms r ON r.id = c.room_id JOIN rooming_houses h ON h.id = r.rooming_house_id WHERE h.landlord_user_id = '10000000-0000-0000-0000-000000000004' AND c.status = 'Active' AND d.status = 'Paid' AND d.payment_transfer_group_id IS NOT NULL AND d.refund_transfer_group_id IS NULL"),
