...
if (args.Length > 0 && args[0] == "verify-showcase-seed")
{
    ...
    var summary = new[]
    {
        ...
        ("xunhuns_invoices_apr_to_jul", "SELECT COUNT(*) FROM invoices i JOIN contracts c ON c.id = i.contract_id JOIN rooms r ON r.id = c.room_id JOIN rooming_houses h ON h.id = r.rooming_house_id WHERE h.landlord_user_id = '10000000-0000-0000-0000-000000000004' AND i.billing_period_start >= DATE '2026-04-01' AND i.billing_period_start <= DATE '2026-07-01'"),
        ("xunhuns_july_draft_invoices", "SELECT COUNT(*) FROM invoices i JOIN contracts c ON c.id = i.contract_id JOIN rooms r ON r.id = c.room_id JOIN rooming_houses h ON h.id = r.rooming_house_id WHERE h.landlord_user_id = '10000000-0000-0000-0000-000000000004' AND i.billing_period_start = DATE '2026-07-01' AND i.status = 'Draft'"),
