...
namespace SmartRentalPlatform.Infrastructure.Persistence.Configurations.Properties
{
    public class PropertyImageConfiguration : IEntityTypeConfiguration<PropertyImage>
    {
        public void Configure(EntityTypeBuilder<PropertyImage> builder)
        {
            ...
            builder.HasIndex(x => new { x.RoomingHouseId, x.SortOrder, x.Id })
                .HasDatabaseName("ix_property_images_house_detail_order");
            builder.HasIndex(x => new { x.RoomId, x.MediaAssetId, x.SortOrder })
                .HasDatabaseName("ix_property_images_room_media_sort");
            builder.HasIndex(x => new { x.RoomId, x.SortOrder, x.Id })
                .HasDatabaseName("ix_property_images_room_detail_order");
            builder.HasIndex(x => new { x.RoomingHouseReviewId, x.SortOrder })
                .HasDatabaseName("ix_property_images_review_sort");
            builder.HasIndex(x => new { x.RoomingHouseReviewId, x.MediaAssetId, x.SortOrder })
                .HasDatabaseName("ix_property_images_review_media_sort");
